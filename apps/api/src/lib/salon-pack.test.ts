...
test('a salon widget answers prices, bookings, and the Bella address', async () => {
  ...
  assert.match(spoken(hello), /Hi Ada/)
  assert.match(spoken(hello), /What would you like done/)
  assert.equal(spoken(hello).includes("What's your name?"), false)
  assert.equal(spoken(hello).includes('voucher'), false)

  const stranger = await machine.run(session('hello', 'there'), graph, 'hello')
  assert.equal(stranger.jumpToFlow, undefined)
  assert.match(spoken(stranger), /Hi, I'm Bella at Bella Hair Studio/)
  assert.match(spoken(stranger), /What's your name\?/)
  assert.equal(spoken(stranger).includes('What would you like done'), false)
  const namedGuest = await reply(machine, stranger, graph, 'Sophie', 'Sophie')
  assert.match(spoken(namedGuest), /Hi Sophie/)
  assert.match(spoken(namedGuest), /What would you like done/)
